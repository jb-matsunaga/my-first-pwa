...
(function() {
  ...
  app.saveSelectedCities = () => {
    ...
    console.log('localStorageに選択した都市リストを登録');
  };
  ...
  if (app.selectedCities) {
    console.log('ユーザー設定があるとき');
    app.selectedCities = JSON.parse(app.selectedCities);
    app.selectedCities.forEach((city) => {
      app.getForecast(city.key, city.label);
      console.log('ユーザー設定をもとにお天気コンテンツ描画');
    });
  } else {
    console.log('初回時、または、ユーザー設定ないとき');
    /* あらかじめ用意したデータで描画（initialWeatherForecast）
     * 本番では、ユーザーのロケーテョンIPを取得してデータを挿入させるのが吉。
     */
    app.updateForecastCard(initialWeatherForecast);
    app.selectedCities = [
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker
             .register('./service-worker.js')//Service Workerを登録
             .then(() => { console.log('Service Worker 登録しました。'); });
  }
